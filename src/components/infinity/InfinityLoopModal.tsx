...
export function InfinityLoopModal({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center bg-black/90 p-0 sm:p-4 sm:pt-[4vh]">
      <div className="flex h-full sm:h-[90vh] w-full max-w-5xl flex-col border border-hard bg-black">
        {/* HEADER */}
        <div className="flex items-center justify-between gap-2 border-b border-hard px-4 py-3">
          <div className="min-w-0 text-[12px] uppercase tracking-widest text-[#c07cff]">
            ♾️ INFINITY_LOOP <span className="text-[#333]">//</span> SELF_IMPROVING_SPEC_ENGINE
          </div>
          <div className="flex shrink-0 items-center gap-2 text-[10px] uppercase tracking-widest">
            ...
            <button onClick={onClose} className="border border-[#333] px-2 py-1 text-[#666] hover:text-white">
              [ESC]
            </button>
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          <section className="border-b border-hard px-4 py-3">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="text-[10px] uppercase tracking-widest" style={{ color: loading ? "#ffaa00" : "#00ff66" }}>
                {loading
                  ? "SCANNING…"
              ...
              {loaded.map((l) => {
                ...
                return (
                  <span
                    ...
                    style={{ borderColor: l.text ? def.color : "#333", color: l.text ? def.color : "#555" }}
                  >
                    {def.icon} {l.path}
                  </span>
                );
          ...
          <section className="border-b border-hard px-4 py-3">
            ...
            <textarea
              ...
              className="w-full resize-y border border-hard bg-[#0a0a0a] p-2 text-[13px] text-[#ddd] outline-none focus:border-[#c07cff]"
            />
            <div className="mt-2 flex flex-wrap gap-2">
              {PRESETS.map((p) => (
                <button
                  ...
                  className="border border-[#333] px-2 py-1 text-[10px] uppercase tracking-widest text-[#888] hover:border-[#c07cff] hover:text-[#c07cff]"
                >
                  [ {p.icon} {p.label} ]
                </button>
              ))}
            ...
            {preAudit.length > 0 && (
              <div className="mt-3 border border-[#ff5500] p-2">
                <div className="mb-1 text-[10px] uppercase tracking-widest text-[#ff5500]">
                  [ ⚠️ PRE_AUDIT — {preAudit.length} PROJECT RULES TOUCH THIS GOAL ]
                </div>
                <ul className="space-y-1">
                  {preAudit.map((f) => (
                    <li key={f.rule.id} className="text-[11px] text-[#ccc]">
                      <span style={{ color: f.rule.severity === "hard" ? "#ff5500" : "#ffaa00" }}>
                        [{f.rule.severity.toUpperCase()}]
                      </span>{" "}
                      {f.rule.text}{" "}
                      <span className="text-[#555]">— {f.rule.source}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {phase === "running" ? (
                ...
                <button
                  ...
                  className="border border-[#c07cff] px-3 py-2 text-[11px] uppercase tracking-widest text-[#c07cff] hover:bg-[#c07cff] hover:text-black disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-[#c07cff]"
                >
              ...
              {err && <span className="text-[11px] text-[#ff5500]">ERR: {err}</span>}
            </div>
          </section>
          ...
          <section className="px-4 py-3">
            ...
            <div className="space-y-2">
              {STAGES.map((s) => {
                ...
                const color = value ? "#00ff66" : isActive ? "#ffaa00" : "#333";
                return (
                  <div key={s.id} className="border" style={{ borderColor: color }}>
                    ...
                    >
                      <span className="min-w-0 truncate text-[11px] uppercase tracking-widest text-white">
                        {s.icon} STAGE {s.n} · {s.label}{" "}
                        <span className="text-[#555]">{s.slash}</span>
                      </span>
            ...
            <section className="border-t border-hard px-4 py-3">
              ...
              <div className="space-y-1">
                {tasks.map((t, i) => (
                  ...
                  >
                    <span className="min-w-0 flex-1 text-[11px] text-[#ccc]">
                      <span className="text-[#555]">{String(i + 1).padStart(2, "0")}</span> {t}
                    </span>
